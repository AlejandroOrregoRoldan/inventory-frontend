import { Package, History, Box, LogOut } from 'lucide-react';
  ...
  return (
    <aside className="w-64 bg-slate-900 text-white flex flex-col h-screen fixed left-0 top-0">
      ...
      <div className="p-4 border-t border-slate-700/50 space-y-2">
        <button
          onClick={onLogout}
          className="w-full flex items-center gap-2 px-4 py-2 rounded-lg text-xs text-slate-400 hover:text-white hover:bg-slate-800 transition-colors"
        >
          <LogOut size={14} />
          Cerrar sesión
        </button>
